...
@Autonomous(name = "Red Auto Far Shoot 6", group = "Examples")
public class redAuto1_newShoot6 extends OpMode {
    ...
    private final Pose startPose = new Pose(84, 6, Math.toRadians(270)); // Start Pose of our robot.
    private final Pose launchPose = new Pose(84, 10, Math.toRadians(270)); // Start Pose of our robot.
    private final Pose order3 = new Pose(99, 30.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose order32 = new Pose(128, 30.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose order2 = new Pose(99, 54.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose order2s = new Pose(104,78.5,Math.toRadians(0));
    private final Pose order21 = new Pose(114, 78.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose order22 = new Pose(128, 54.5, Math.toRadians(0));
    private final Pose park = new Pose(104, 6, Math.toRadians(0));
    private final Pose order1s = new Pose(104,54.5,Math.toRadians(0));
    private final Pose order11 = new Pose(108, 54.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose order12 = new Pose(116, 54.5, Math.toRadians(0)); // Middle (Second Set) of Artifacts from the Spike Mark.
    ...
    public void autonomousPathUpdate() throws InterruptedException {
        ...
        switch (pathState) {
            ...
            case 0:
                ...
                if (hasShot && !shooting) {
                    hasShot = false;   // reset for next shot
                    setPathState(1);
                }
                break;

            // ======================================
            // DRIVE TO FIRST COLLECTION LINE
            // ======================================
            case 1:
                intake.setPower(1);
                moveTurretTo(210);
                follower.followPath(moveToOrder3); // straight line
                setPathState(2);
                break;
            ...
            case 2:
                if (!follower.isBusy()) {
                    moveTurretTo(210);
                    follower.followPath(moveToOrder32); // sideways
                    setPathState(3);
                }
                break;

            // ======================================
            // RETURN TO LAUNCH FOR SECOND SHOT
            // ======================================
            case 3:
                if (!follower.isBusy()) {
                    moveTurretTo(210);
            ...
            // ======================================
            // RETURN TO LAUNCH FOR FINAL SHOT
            // ======================================
            case 7:
                if (!follower.isBusy()) {
                    moveTurretTo(210);
    ...
    public void updateShooting() {
        ...
        else if (shootStep == 1 && shootTimer.milliseconds() >= 600) {
            arm1.setPosition(0);
            shootTimer.reset();
            shootStep = 2;
        }

        else if (shootStep == 2 && shootTimer.milliseconds() >= 600) {
            arm2.setPosition(1);
            shootTimer.reset();
            shootStep = 3;
        }

        else if (shootStep == 3 && shootTimer.milliseconds() >= 600) {
            arm2.setPosition(0);
            shootTimer.reset();
            shootStep = 4;
        }

        else if (shootStep == 4 && shootTimer.milliseconds() >= 600) {
            arm3.setPosition(1);
            shootTimer.reset();
            shootStep = 5;
        }

        else if (shootStep == 5 && shootTimer.milliseconds() >= 600) {
            arm3.setPosition(0);
            shooting = false;
            shootStep = 0;
        }
